...
char* mass1[5] = { "Anton","Nikita","Artur","Lena","Nastya" };
char* mass2[5] = { "Petrov","Ivanov","Sidorov","Vareev","May" };
char* mass3[4] = { "MK-101","MK-102","MK-201","MK-202" };
...
Student* createrandomstudent() {
	...
	student->Age = rand() % 20 + 20;

	student->Number = rand();

	for (int i = 0; i < 5; i++)
		student->Marks[i] = rand() % 3 + 2;
